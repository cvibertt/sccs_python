...
def print_smoothagesccs(x, digits=4, signif_stars=True, **kwargs):
    """
    Print summary for smoothed age SCCS model.
    
    Parameters:
    x : fitted model object or summary
        Should have attributes like coefficients, conf.int, concordance, etc.
    """
    # Assume x is already summarized or summarize it
    ...
    print("Spline based age relative incidence function:")
    print(f"Smoothing parameter = {x.smoothingpara}")
    print(f"Cross validation score = {x.cv:.2f}")